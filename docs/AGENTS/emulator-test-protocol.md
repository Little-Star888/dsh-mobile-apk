# emulator-test-protocol.md — 模拟器验收规范（CDP 断言 + adb 用户级操作，双轨必跑）

> 本文件是 `AGENTS.md` §2「模拟器测试规范」的**详档**：AGENTS.md 只放强制条款，命令与判据在这里。
> 一句话：**只跑 CDP 不算验收**——用户看得见的东西，必须在模拟器上用 adb 真点一次、真截一张图。

---

## 1. 适用范围（什么时候必须双轨）

**必须双轨**（A 轨 CDP 断言 + B 轨 adb 用户级操作）——凡改动会让用户**看见或摸到**不同：
...
判 **pass** 的充要条件：**A 轨全绿 且 B 轨每步截图与预期一致**（人工判读并写进结论表）。
缺任何一轨 = **未验收**，不得在 PR/提交里写「功能完好」。失败时追加：logcat 片段 + 复现步骤（哪台设备、哪个方向、第几步）。
