...
117. **自动落位必须「边沿触发」，不能「电平触发」（0.14.0 用户实报两连）**：监听壳侧状态来「把面板切到前台」
   时，若写成每 N 秒无条件 `openTab(...)`（电平触发 = 持续断言「只要页面存在就永远置前」），会同时制造两个
   用户可见缺陷：**① 用户一收起侧栏就被下一拍拽开**；**② 点 × 关掉后又被切回来，表现为「关不掉」**。
   实测判据：把该轮询临时停掉（例如让互斥声索记录指向别家）后，收起态 `collapsed:true` + `visible:false`
   能稳定保持——反证轮询就是元凶。正确结构 = **只在页面签名（tabs + pageGeneration）变化时动作一次**；
   首次观测只建立基线不动作。

118. **`openTab` 本身会展开侧栏——「不强制展开」不能靠它实现（0.14.0 源码实证）**：
   `dsh-client-ui-sidebar-right/lib/client.js` 的 `openContent` 第一条 op 就是 `planSetExpanded(state, true)`
...
123. **双树镜像的推送顺序会造成 coord CI 假红（0.14.0 反复踩到）**：coord 仓 CI 的「镜像一致性门禁」
   会 clone apk 仓的对端分支再逐字节比对（check-patch-mirror --require-peer）。这是两个仓库两次独立
   推送；若先推 coord，coord CI 完全可能在 apk 的镜像 commit 落地前就去 clone，必然报「内容漂移」，
   看起来像真缺陷，实际是窗口竞态。**本地 check-patch-mirror 恒 PASS 就是判据**：本地过而 CI 红
   ⇒ 先查推送顺序，不要先去改代码。
   正确顺序：**先推镜像方（apk），等远端可见（约 15s），再推权威方（coord）**。
   已固化为 scripts/push-both.ps1（两仓均可调用，按序推送 + 逐次重试）。

124. **虚拟屏四件事：序号复用 / 资源全局唯一 / 销毁不设锁 / 空闲回收（0.14.0 用户实报）**：
   ① **序号必须复用**：旧实现每建一块 nextAliasIndex += 1 且从不回收，反复建/销毁得到
   virtual-1 → 2 → 3 …（实测复现）；上限只有 1 块时界面显示「虚拟屏 3」却切不回更早那块，
   与「编号 = 当前屏幕标识」的直觉冲突。分配必须扫描**最小空闲序号**。
   ② **资源全局唯一，归属只管呈现**：虚拟屏是稀缺系统资源（本版上限 1 块），隔离的是「可见性/操作」，
   不是「资源本身」。把幂等判断写成「本会话是否已有屏」会制造新缺陷：面板路径创建不传 session
   （owner=null），它先建屏后，模型路径（带 session）会被算作「本会话没有屏」→ 建第 2 块 → 撞上限。
   幂等必须看 records.size。（把稀缺资源按租户切分 ⇒ 上限被除以租户数。）
   ③ **销毁不得按归属拒绝**：用户明确要求「别忘了给模型销毁权限，否则没人能关掉了」。若只允许创建者
   销毁，创建它的对话被关闭后那块屏就**永远没人能关**（只剩设置页强制销毁一条路）。
   ④ **空闲回收**：此前**没有任何回收路径**，只有显式 vdDestroy / 设置页强制销毁。虚拟屏持有
   display + ImageReader + HandlerThread，是长期占位的真实资源。已加 reclaimIdle()（阈值 10 分钟，
   由 vd op 入口 + 前台每 2 分钟扫描触发）。
