...
# 旧串在场时 git --version 一样正常输出，但一切经 shell 的 git 路径全废
# （credential.helper / `!` 前缀 alias / hook / rebase --exec）。
if grep -q -- "/data/data/com.termux/files/usr/bin/sh" "$B/bin/git.real" 2>/dev/null; then
  echo "P4b git SHELL_PATH: FAIL（git.real 内仍含旧前缀串——build-snapshot-013.mjs 第 7d2 步的等长替换未生效）"
  FAIL=1
else
  echo "P4b git SHELL_PATH: PASS（无旧前缀残留）"
fi
# 功能断言：不需要网络、不需要凭据，只验证 helper 能被 shell 拉起来。
# `-c core.askPass=` 必须给：helper 执行失败时 git 会回退到 askpass，而 askpass
# 可能返回环境里真实存在的凭据 —— 既会让本断言假绿，又会把凭据打印出来。
if [ -x "$B/bin/git" ]; then
